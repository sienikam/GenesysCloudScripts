...
# Check if DeleteRecordingsFromQueue.csv exists
if [[ ! -f DeleteRecordingsFromQueue.csv ]]; then
  echo "Error: DeleteRecordingsFromQueue.csv not found. Exiting...\n"
  exit 1
fi

while IFS=";" read -r interval actiondate queue
do
  echo "Interval: $interval"
  echo "Actiondate: $actiondate"
  echo "Queue: $queue"
  echo -e "Need to obtain the id of queue "$queue"...\n"
  queueid=$(gc routing queues list -a --clientid $oauthclient_id --clientsecret $oauthclient_secret --environment $environment | jq -r '.[] | select(.name == "'"$queue"'") | .id' | tr -d '\r')
...
done <<< "$(tail -n +2 DeleteRecordingsFromQueue.csv)"
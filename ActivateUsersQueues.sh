  ...
  then
        ...
        do
          ...
          then
                ...
                echo '[{"id":"'"$UserID"'","joined":true}]' | gc routing queues members activate $QueueID --clientid $oauthclient_id --clientsecret $oauthclient_secret --environment $environment
          fi
         done
  fi
  echo
done <<< "$(tail -n +2 ActivateUsersQueues.csv)"

...
# Check if DeleteRecordingsFromDivision.csv exists
if [[ ! -f DeleteRecordingsFromDivision.csv ]]; then
  echo "Error: DeleteRecordingsFromDivision.csv not found. Exiting...\n"
  exit 1
fi

while IFS=";" read -r interval actiondate division
do
  echo "Interval: $interval"
  echo "Actiondate: $actiondate"
  echo "Division: $division"
  echo -e "Need to obtain the id of division "$division"...\n"
  divisionid=$(gc authorization divisions list -a --clientid $oauthclient_id --clientsecret $oauthclient_secret --environment $environment | jq -r '.[] | select(.name == "'"$division"'") | .id' | tr -d '\r')
...
done <<< "$(tail -n +2 DeleteRecordingsFromDivision.csv)"
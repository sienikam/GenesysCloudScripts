...
# Check if DeleteRecordingsPerUser.csv exists
if [[ ! -f DeleteRecordingsPerUser.csv ]]; then
  echo "Error: DeleteRecordingsPerUser.csv not found. Exiting...\n"
  exit 1
fi

while IFS=";" read -r interval actiondate useremail
do
  echo "Interval: $interval"
  echo "Actiondate: $actiondate"
  echo "User: $useremail"
  echo -e "Need to obtain the id of user "$useremail"...\n"
  userid=$(gc users list -a --clientid $oauthclient_id --clientsecret $oauthclient_secret --environment $environment | jq -r '.[] | select(.email == "'"$useremail"'") | .id' | tr -d '\r')
...
done <<< "$(tail -n +2 DeleteRecordingsPerUser.csv)"
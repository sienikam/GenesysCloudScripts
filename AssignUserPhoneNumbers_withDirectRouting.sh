...
if [[ ! -f AssignUserPhoneNumbers_withDirectRouting.csv ]]; then
  echo -e "\nError: File AssignUserPhoneNumbers_withDirectRouting.csv not found. Exiting...\n"
  exit 1
fi
...
while IFS=";" read -r useremail primary work1 work2 work3 mobile direct
do
...
done <<< "$(tail -n +2 AssignUserPhoneNumbers_withDirectRouting.csv)"
rm UsersList.json